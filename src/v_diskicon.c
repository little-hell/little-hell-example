...
static void CopyRegion(pixel_t *dest, int dest_pitch, pixel_t *src, int src_pitch, int w,
                       int h)
{
    pixel_t *s, *d;
    int y;
...
static void SaveDiskData(const char *disk_lump, int xoffs, int yoffs)
{
    ...
    tmpscreen =
        Z_Malloc(SCREENWIDTH * SCREENHEIGHT * sizeof(*tmpscreen), PU_STATIC, NULL);
    ...
    disk_data =
        Z_Malloc(LOADING_DISK_W * LOADING_DISK_H * sizeof(*disk_data), PU_STATIC, NULL);
...
void V_EnableLoadingDisk(const char *lump_name, int xoffs, int yoffs)
{
    ...
    saved_background = Z_Malloc(
        LOADING_DISK_W * LOADING_DISK_H * sizeof(*saved_background), PU_STATIC, NULL);
...
void V_DrawDiskIcon(void)
{
    if (disk_data != NULL && recent_bytes_read > diskicon_threshold)
    {
        ...
        CopyRegion(saved_background, LOADING_DISK_W, DiskRegionPointer(), SCREENWIDTH,
...
void V_RestoreDiskBackground(void)
{
    if (disk_drawn)
    {
        ...
        CopyRegion(DiskRegionPointer(), SCREENWIDTH, saved_background, LOADING_DISK_W,
                   LOADING_DISK_W, LOADING_DISK_H);
